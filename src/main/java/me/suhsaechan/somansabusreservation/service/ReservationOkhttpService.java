...
public class ReservationOkhttpService {
  ...
  public boolean makeReservation(int passengerId, BusRoute busRoute) {
    ...
    Request request = new Request.Builder()
        ...
        .addHeader("Referer", "https://cs.android.busin.co.kr/driving/ride_on.aspx?rt=r&disptid=" + busRoute.getDisptid() + "&shuttletype=" + busRoute.getCaralias() + "&scr=0&isshuttle=0")
        .build();

    try (Response response = client.newCall(request).execute()) {
      if (!response.isSuccessful()) {
        log.error("예약 실패, 코드: {}", response.code());
        return false;
      }
